...
const previewDocument = (example: SiteExample): string => example.preview.html
  .replace(/<link rel="stylesheet" href="styles\.css"\s*\/?>/, `<style>${example.preview.css}</style>`)
  .replace(/<script type="module" src="app\.js"><\/script>/, `<script type="module">${example.preview.js.replaceAll("</script", "<\\/script")}</script>`);
