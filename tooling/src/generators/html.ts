...
function componentMarkup(compiled: CompiledExample, component: CompiledComponent): string {
  ...
  if (component.role === "primary_action" || component.role === "button") {
    ...
    return `<button class="ui-button ui-button-primary" type="button" data-component="${id}" data-action="submit-payment" aria-label="${escapeHtml(label)}"><span class="ui-button-label">${escapeHtml(text)}</span><span class="ui-button-spinner" aria-hidden="true"></span></button>`;
  }
...
export function generateHtmlArtifact(compiled: CompiledExample): HtmlArtifact {
  ...
  const html = `<main class="mobile-screen" data-example-id="${escapeHtml(compiled.example_id)}" data-state="default" aria-labelledby="screen-title"><div class="safe-area-top" aria-hidden="true"></div><div class="mobile-scroll"><header class="screen-header"><p class="eyebrow">${escapeHtml(String(compiled.classification.app_category ?? "mobile"))}</p><h1 id="screen-title">${escapeHtml(title)}</h1><p class="screen-description">${escapeHtml(String(compiled.request.user_job ?? "Complete the task"))}</p></header><form class="screen-content" data-component="${escapeHtml(screen?.id ?? "screen")}" novalidate>${children}<section class="fixture-card" aria-label="Payment method"><div><span class="fixture-label">Payment method</span><strong>Visa ···· 4242</strong></div></section></form></div><div class="mobile-sticky" data-fixed-region="primary-action"><div class="sticky-inner"><button class="ui-button ui-button-primary" type="button" data-action="submit-payment" aria-label="${escapeHtml(contentText(compiled, "pay", "Pay now"))}"><span class="ui-button-label">${escapeHtml(contentText(compiled, "pay", "Pay now"))}</span><span class="ui-button-spinner" aria-hidden="true"></span></button></div></div><div class="live-region" aria-live="polite" role="status"></div></main>`;
  const css = `:root { --safe-top: env(safe-area-inset-top, ${safeTop}px); --safe-bottom: env(safe-area-inset-bottom, ${safeBottom}px); --screen-inset: ${screenInset}px; --component-gap: ${componentGap}px; --section-gap: ${sectionGap}px; --card-padding: ${cardPadding}px; --control-height: ${controlHeight}px; --corner-radius: ${radius}px; --sticky-region-height: ${sticky}px; --scroll-content-bottom-inset: ${bottomInset}px; --color-text: ${color(compiled, "text", "#111827")}; --color-surface: ${color(compiled, "surface", "#ffffff")}; --color-primary: ${color(compiled, "primary", "#2563eb")}; --color-danger: ${color(compiled, "danger", "#dc2626")}; }\n${BASE_CSS}`;
