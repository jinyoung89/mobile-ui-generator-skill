...
const runtime = `(() => {
  ...
  function bootFromLocation(root) { const name = new URLSearchParams(window.location.search).get('profile') || 'standard'; const profile = applyBrowserProfile(root, name); const screen = root.querySelector('.mobile-screen') || root; const live = screen.querySelector('.live-region'); const setState = state => { screen.dataset.state = state; if (live) live.textContent = state === 'success' ? '완료되었습니다.' : state === 'loading' ? '처리 중입니다.' : ''; }; screen.querySelectorAll('[data-action]').forEach(button => button.addEventListener('click', () => { if (screen.dataset.state === 'success') return; setState('loading'); window.setTimeout(() => setState('success'), 80); })); screen.querySelectorAll('input[data-focus-scroll]').forEach(input => input.addEventListener('focus', () => window.setTimeout(() => input.scrollIntoView({ block: 'center', behavior: 'smooth' }), 0))); return profile; }
