...
struct CommerceCheckoutAddressScreen: View {
  ...
  var body: some View {
    ScrollView { VStack(alignment: .leading, spacing: 12) {
      ...
      proofCard(model.item_title, model.amount); TextField(model.field_label, text: $input).textFieldStyle(.roundedBorder).frame(minHeight: 52).focused($focused); proofCard(model.secondary_label, model.item_meta)
